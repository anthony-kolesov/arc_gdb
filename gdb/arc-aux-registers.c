...
static const struct gdb_xml_element field_children[] = {
  {"meaning", meaning_attributes, NULL,
   GDB_XML_EF_OPTIONAL | GDB_XML_EF_REPEATABLE, start_meaning, NULL},
  ELEMENT_END_MARKER
};


static const struct gdb_xml_element register_children[] = {
  {"field", field_attributes, field_children,
   GDB_XML_EF_OPTIONAL | GDB_XML_EF_REPEATABLE, start_field, NULL},
  ELEMENT_END_MARKER
};


static const struct gdb_xml_element bcr_children[] = {
  {"bcrfield", bcrfield_attributes, NULL,
   GDB_XML_EF_OPTIONAL | GDB_XML_EF_REPEATABLE, start_bcrfield, NULL},
  ELEMENT_END_MARKER
};


static const struct gdb_xml_element feature_children[] = {
  {"register", aux_register_attributes, register_children,
   GDB_XML_EF_OPTIONAL | GDB_XML_EF_REPEATABLE, start_register, NULL},
  {"bcr", aux_register_attributes, bcr_children,
   GDB_XML_EF_OPTIONAL | GDB_XML_EF_REPEATABLE, start_bcr, NULL},
  {"ecr", core_register_attributes, NULL,
   GDB_XML_EF_OPTIONAL | GDB_XML_EF_REPEATABLE, start_ecr, NULL},
  ELEMENT_END_MARKER
};
...
static const struct gdb_xml_element target_children[] = {
  {"architecture", NULL, NULL, GDB_XML_EF_OPTIONAL, start_architecture,
   end_architecture},
...
static const struct gdb_xml_element elements[] = {
  {"target", target_attributes, target_children, GDB_XML_EF_NONE, start_target,
   NULL},
  ELEMENT_END_MARKER
};
  ...
  if ((fd = open (name, O_RDONLY)) != -1)
      ...
      if ((status = fstat (fd, &stat)) != -1)
	{
	  ...
	  if ((contents = xmalloc (size + 10)))
	    ...
	    warning (_
		     ("can not allocate bufffer to hold contents of file '%s'"),
		     name);
	}
      else
...
//  ParsingData* data = user_data;
  ...
  if (strcmp (version, "1.0") != 0)
    gdb_xml_error (parser,
		   _
		   ("Auxiliary registers description has unsupported version \"%s\""),
		   version);
		  ...
		  VEC (gdb_xml_value_s) * attributes, int is_BCR)
{
  ...
  for (i = 0; i < info->aux_register_count; i++)
	...
	if (strcasecmp (reg->name, info->aux_registers[i].name) == 0)
	  {
	    warning (_
		     ("auxiliary register with name '%s' already defined in file %s"),
	...
	{
	  warning (_
		   ("auxiliary register with number %u already defined in file %s"),
  ...
  if (add)
	...
	&info->aux_registers[info->aux_register_count - 1];
	   ...
	   void *user_data, VEC (gdb_xml_value_s) * attributes)
{
  ...
  if ((ARC_FIRST_EXT_CORE_REGNUM <= number)
	...
	{
	  warning (_
		   ("extension core register with number %d already defined in file %s"),
    ...
    warning (_
	     ("extension core register with invalid number %d defined in file %s"),
	     number, data->filename);
}
	   ...
	   void *user_data, VEC (gdb_xml_value_s) * attributes, int is_BCR)
{
  ...
  for (i = 0; i < length; i++)
	...
	field->description = xstrdup (value);
      else if (NAME_IS ("offset"))
	field->offset = (unsigned int) *(ULONGEST *) value;
  ...
  if (is_BCR)
	...
	{
	  warning (_
		   ("field '%s' is readable in write-only auxiliary register '%s' in file %s"),
	...
	{
	  warning (_
		   ("field '%s' is writable in read-only auxiliary register '%s' in file %s"),
		   field->name, reg->name, data->filename);
	  add = FALSE;
	}
    }
  ...
  if (field->size == 0)
    {
      warning (_
	       ("field '%s' contains no bits in auxiliary register '%s' in file %s"),
	       field->name, reg->name, data->filename);
      add = FALSE;
    }
  ...
  if (field->offset > BITS_IN_REGISTER - 1)
    {
      warning (_
	       ("field '%s' offset > %u in auxiliary register '%s' in file %s"),
  ...
  if (field->offset + field->size > BITS_IN_REGISTER)
    {
      warning (_
	       ("field '%s' is too wide in auxiliary register '%s' in file %s"),
	       field->name, reg->name, data->filename);
      add = FALSE;
    }
  ...
  for (i = 0; i < reg->field_count; i++)
	...
	if (strcasecmp (field->name, f->name) == 0)
	  {
	    warning (_
		     ("field '%s' already defined in auxiliary register '%s' in file %s"),
		     field->name, reg->name, data->filename);
	    add = FALSE;
	  }
	...
	{
	  warning (_
		   ("field '%s' overlaps field '%s' in auxiliary register '%s' in file %s"),
  ...
  if (field->value_for_write > data->maxFieldContents)
    {
      warning (_
	       ("value on write %u is too large for %u-bit reserved field in auxiliary register '%s' in file %s"),
...
//  }
  ...
  if (meaning.value > data->maxFieldContents)
    {
      warning (_
	       ("value %u is too large for field '%s' in auxiliary register '%s' in file %s"),
  ...
  for (i = 0; i < field->meaning_count; i++)
	...
	{
	  warning (_
		   ("meaning '%s' already defined for field '%s' in auxiliary register '%s' in file %s"),
	...
	{
	  warning (_
		   ("value %u already has meaning for field '%s' in auxiliary register '%s' in file %s"),
		   meaning.value, field->name, reg->name, data->filename);
	  add = FALSE;
	}
    }
...
parse_XML (const char *document,
	   xml_fetch_another fetcher, void *fetcher_baton, ParsingData * data)
{
	       ...
	       int replace, int inform, int check)
{
  ...
  if (xml)
	...
	(void) make_cleanup (xfree, dirname);
      ...
      if (inform)
	printf_filtered (_
			 ("Auxiliary register definitions read from file %s\n"),
			 filename);
...
#define DESCR  "auxiliary registers definition file "
  ...
  if (access (AUX_REG_DEFINITION_FILE, F_OK) == 0)
    {
      if (!read_XML_file
	  (AUX_REG_DEFINITION_FILE, gdbarch, FALSE, inform, check))
	...
	{
	  char *home_file =
	    xstrprintf (_("%s/%s"), home_dir, AUX_REG_DEFINITION_FILE);
	  ...
	  if (access (home_file, F_OK) == 0)
	    ...
	    warning (_
		     ("can not find " DESCR AUX_REG_DEFINITION_FILE
		      " in either current directory or $HOME"));

	  xfree (home_file);
	}
      else
	warning (_
		 ("HOME environment variable is not set - can not find " DESCR
		  AUX_REG_DEFINITION_FILE));
    }
}
...
find_info (int must_be_defined)
{
...

  {
	...
	if (info->aux_register_count == 0)
	  error (_
		 ("no auxiliary registers have yet been defined for this target"));
      }
...
static void
simulator_mapping (int gdb_regno, int *hw_regno, ARC_RegisterClass * reg_class)
{
  *hw_regno = -1;
...
arc_aux_reg_read_command (char *arg, int from_tty)
{
  ...
  if (arg2)
	...
	{
	  warning (_(AUX_REG_READ_COMMAND ": %s < %s, showing one register"),
		   arg2, arg);
...
arc_aux_reg_write_command (char *arg, int from_tty)
{
  ...
  if (p == NULL)
    {
      printf_filtered (_
		       (AUX_REG_WRITE_COMMAND ": no second argument\n"
...
void
arc_aux_pc_guard (struct gdbarch *gdbarch)
{
...
arc_aux_check_pc_defined (struct gdbarch *gdbarch)
{
  ...
  if (arc_aux_pc_number (gdbarch) < 0)
    error (_
	   ("There is no auxiliary register description for the PC (Program Counter)"));
			      ...
			      unsigned int defaultNumber)
{
  ...
  if (def == NULL)
    {
      warning (_
	       ("there is no auxiliary register description for the %s register - "
...
_initialize_arc_aux_regs (void)
{
  ...
  (void) add_cmd (AUX_READ_EXTRA_FILE_COMMAND,
		  ...
		  _
		  ("Read a file describing an additional set of auxiliary registers.\n"
  ...
  (void) add_cmd (ARC_BCR_COMMAND,
		  ...
		  ("Show Build Configuration Registers in the ARC processor variant.\n"
		   ARC_BCR_COMMAND_USAGE), &infolist);
